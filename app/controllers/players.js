...
var sendConfig = function (player, group, periodic) {
    ...
    //generate license file automatically and save to license folder
    var cpuId = player.cpuSerialNumber;
    var newLicenseData = createLicense(cpuId, installation);
...
function readLicense(cpuSerialNumber, crypted){

  //define decryption key based off of pi cpuSerialNumber and secret (used to encrypt and decrypt)
  var str = cpuSerialNumber; //Serial of Pi
  var secret = "pisignageLangford"; //shared secret
  var key = crypto.createHmac("sha1", secret).update(str).digest("hex"); //generate key from cpuserial and sharedsecret

  //create the decipher based on algo and key
  var decipher = crypto.createDecipher(algo, key);

  //decrypt our encrypted ata with the decipher
...
function createLicense(cpuSerialNumber, piSignageUsername){

  //build our License/Json object data
  var data = JSON.stringify({ enabled: true, generatedOn: new Date(), validity:0, installation: piSignageUsername });
  ...
  //create the cipher based on algo and key
  var cipher = crypto.createCipher(algo, key);

  //encrypt our jason string data using the cipher
  var encrypted = cipher.update(data, "utf8", "hex");
  encrypted += cipher["final"]('hex');


  console.log('Created license file for ' + cpuSerialNumber);
  fs.writeFile(path.join(config.licenseDirPath + installation + "\\license_" + cpuSerialNumber + '.txt'), encrypted, "UTF8", function(err){
    if(err){
